...
/**
 * Sample implementation of {@link SimulationObserver}. This simulation observer
 * simply prints out the notification info and keeps count of how many
 * notification it receive.
 * 
...
public class PrintingObserverExt implements SimulationObserverExt {
	...
	public void notifyFlow(String type, String name, double inFlow, double... outFlow) {
		...
		if (exists(inFlow))
			System.out.println("\t-> in flow= " + inFlow);
		if (exists(outFlow))
			System.out.println("\t<- out flow= " + Arrays.toString(outFlow));
		countNotifications++;
	}
